...
def set_gpu(idx: int = 0) -> bool:

    """Limit TensorFlow to a single GPU (chosen by PCI‑bus order)."""
    ...
    if not gpus:
        ...
        print("No GPU detected.");               return False

    if not 0 <= idx < len(gpus):

        print(f"GPU index {idx} out of range.");  return False
    ...
    try:
        ...
        print(f"Using GPU {idx}: {gpus[idx].name}")

        return True

    except Exception as e:

        print(f"Failed to set GPU {idx}: {e}")

        return False


def set_gpu_old(gpu_index):
    '''
...
def get_model(input_shape, model_name):
    ...
    if 'rajpurkar' in model_name:
        ...
        # Convolutional layer configuration
        conv_config = {'kernel_size': 10,
                    'filters': 128,
                    'activation': 'relu'}

        # Block A
        layers = Conv1D(**conv_config)(input_layer)
        layers = Conv1D(**conv_config)(layers)
        layers = MaxPooling1D(pool_size = pool_size)(layers)
        layers = Dropout(dropout_rate)(layers)
        layers = BatchNormalization()(layers)
    ...
    elif model_name == 'baloglu':
        initializer = 'he_normal'
        dropout_rate = 0.2
        pool_size = 2

        # Bloco 1
        layers = Conv1D(filters=64, kernel_size=5, strides=1, activation='relu')(input_layer)
        layers = Conv1D(filters=128, kernel_size=3, strides=1, activation='relu')(layers)
        layers = MaxPooling1D(pool_size=pool_size, strides=2)(layers)
        layers = Dropout(dropout_rate)(layers)
        
        # Bloco 2
        layers = Conv1D(filters=128, kernel_size=13, strides=1, activation='relu')(layers)        
        layers = Conv1D(filters=256, kernel_size=7, strides=1, activation='relu')(layers)
        layers = MaxPooling1D(pool_size=pool_size, strides=2)(layers)
...
def plot_cm(confusion_matrix, class_names, fontsize = 10, cmap = 'Blues'):
    ...
    # Subclasses configuration
    # sns.heatmap(df_cm, annot = False, square = True, fmt = '.1f', 
    #             # linewidths = 0.5, linecolor='lightgray',
    #             cbar = True, annot_kws = {"size": fontsize}, cmap = cmap, ax = ax,
    #             xticklabels = class_names, yticklabels = class_names)
    ...
    # Subclasses configuration
    # ax.set_xticklabels(xticks, rotation=90) # Rotating the y-ticks
...
def get_mlcm_report(conf_mat, target_names, model_name, dataset):
    ...
    print(d['weighted avg']) #alterei essa linha
