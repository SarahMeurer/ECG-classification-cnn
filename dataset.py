import ast          # Transform strings into objects from ast.literal_eval
import numpy as np  # Basic matrix manipulations
import pandas as pd # Manipulate the .csv files
import pathlib      # Path manipulation
import tqdm         # Measure the progress
import wfdb         # Extract the ECG signal

'''

    This script collects the information in the PTB-XL database and separate in
    'train', 'validation' and 'test' information.
...
def load_raw_data(df, data_folder, sampling_rate):
    ...
    # Get information about one example file for 100Hz sampling.
    if sampling_rate == 100:
        _, info = wfdb.rdsamp(str(data_folder / df.filename_lr[0]))
        selected = df.filename_lr

    # Get information about one example file for 500Hz sampling.
    ...
    # Raising an error for invalid sampling_rate value 
        ...
        raise ValueError('Wrong samplig_rate value.')
...
# Defining path and choosing the sampling rate
path = '/datasets/ptbxl'    # path to the PTB-XL database
path = pathlib.Path(path)   # creating the pathlib.PosixPath variable
...
# Load raw ECG data
X = load_raw_data(metadata, path, sampling_rate)
